## 搭建方法
1. 从[@BotFather](https://t.me/BotFather)获取token，并且可以发送`/setjoingroups`来禁止此Bot被添加到群组，选择bot然后`disable`
...
4. 登录[cloudflare](https://workers.cloudflare.com/)，创建一个worker，`Workers 和 Pages`→`创建`→`hello world`
5. 配置worker的变量
...
7. 点击左上`编辑代码`，复制[这个文件](./worker.js)到编辑器中
8. 通过打开`https://*左上角查看此Workers*.workers.dev/registerWebhook`来注册websoket
